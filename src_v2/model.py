...
def attn(x, scope, n_state, *, past, hparams):
    ...
    assert n_state % hparams.get("n_head") == 0
    if past is not None:
        assert past.shape.ndims == 5  # Should be [batch, 2, heads, sequence, features], where 2 is [k, v]

    def split_heads(x):
        # From [batch, sequence, features] to [batch, heads, sequence, features]
        return tf.transpose(split_states(x, hparams.get("n_head")), [0, 2, 1, 3])
...
def past_shape(*, hparams, batch_size=None, sequence=None):
    return [batch_size, hparams.get("n_layer"), 2, hparams.get("n_head"), sequence, hparams.get("n_embd") // hparams.get("n_head")]
...
def model(hparams, X, past=None, scope='model', reuse=False):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        wpe = tf.compat.v1.get_variable('wpe', [hparams.get("n_ctx"), hparams.get("n_embd")],
                             initializer=tf.compat.v1.random_normal_initializer(stddev=0.01))
        wte = tf.compat.v1.get_variable('wte', [hparams.get("n_vocab"), hparams.get("n_embd")],
        ...
        # Transformer
        presents = []
        pasts = tf.unstack(past, axis=1) if past is not None else [None] * hparams.get("n_layer")
        assert len(pasts) == hparams.get("n_layer")
        ...
        h_flat = tf.reshape(h, [batch*sequence, hparams.get("n_embd")])
        logits = tf.matmul(h_flat, wte, transpose_b=True)
        logits = tf.reshape(logits, [batch, sequence, hparams.get("n_vocab")])
